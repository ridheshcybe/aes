...
function crypt(encrypt, Text, key, IV) {
    ...
    if (!encrypt) {
        if (!Text.includes("(IVSPLIT)")) throw new Error("IV SHOULD BE INCLUDED");
    ...
    if (IV.length != 32) throw new Error('JS-Rijndael crypt: IV must be 32 bytes long for 256 encryption Got:' + JSON.stringify({ Len: IV.length, iv: IV }));
