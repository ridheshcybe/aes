...
function shiftRows(state: number[], shifttab: number[]): void {
...
function mixColumns_Inv(state: number[]): void {
  for (let i = state.length - 4; i >= 0; i -= 4) {
    const s0 = state[i + 0];
    const s1 = state[i + 1];
    const s2 = state[i + 2];
...
function cryptEnc(block: number[], strkey: string): void {
...
function cryptDec(block: number[], strkey: string): void {
  const bB = block.length;
  let bBi = 0;
  let kBi = 0;
...
export function encrypt(textin: string, IV: string, key: string, cipher: cipherTypes): string {
  ...
  const text = textin.split("").map((e) => e.charCodeAt(0));
  ...
  if (!IV) throw new Error(`crypt: IV is required for mode: CBC`);
  if (IV.length != blockS)
    throw new Error(`crypt: IV must be ${blockS} bytes long for ${cipher}`);
  const iv = IV.slice(0)
    .split("")
    .map((e) => e.charCodeAt(0));
  ...
  while (text.length < chunks * chunkS) text.push(0);
  ...
  return `${out.map((e) => String.fromCharCode(e)).join("")}:():${IV}`;
}

export function decrypt(combined: string, key: string, cipher: cipherTypes): string {
  let out: number[] = [];
  const split = combined.split(":():");
  const textin = split[0];
  const IV = split[1];
  if (!textin) throw new Error("Crypt: textin not possible");
  ...
  out.push(last);
  return out.map((e) => String.fromCharCode(e)).join("");
}
